		...
		<div id="" class="" _v-3a85f76e="" style="">
			<div class="scroller" _v-3a85f76e="" style="transition-property: transform; transform-origin: 0px 0px 0px; transform: translate(0px, 0px) scale(1) translateZ(0px);">
				<div class="weui_cells_title" _v-3a85f76e="">
					<?php if($timeSpan): ?>预约记录<?php else: ?>暂无挂号记录<?php endif; ?>
				</div>   
				<div class="weui_cells weui_cells_access" _v-3a85f76e="">
					<!--v-for-start-->    
					<?php if(is_array($timeSpan)): foreach($timeSpan as $key=>$vo): ?><a class="weui_cell" href="/index.php/Home/Index/checkAppointDetail/appointId/<?php echo ($vo['id']); ?>" _v-3a85f76e="">
					...
					<div class="weui_cell_bd weui_cell_primary" _v-3a85f76e="">  
						<p _v-3a85f76e="">
							<?php echo ($vo['realInfo']['visitdate']); ?> <?php echo ($vo['deptInfo']['deptname']); ?> <?php echo ($vo['docInfo']['doctorname']); if($vo["status"] == 3): ?><span style="color:gray">（已取消）</span><?php elseif($vo["status"] == 2): ?><span style="color:red">（失败）</span><?php else: ?><span style="color:green">（成功）</span><?php endif; ?>   
						</p>
					</div>
